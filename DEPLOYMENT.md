...
#### 3.3. Подключение локального фронтенда к бэкенду на сервере

Если вы запускаете фронтенд **локально на своём ПК**, а бэкенд работает **на удалённом сервере**, укажите URL сервера в вашем `.env`:

1. Откройте файл `.env` в папке `frontend/vite-project/` (если его нет — создайте).

2. Задайте или измените переменную, подставив IP или домен вашего сервера:

   ```
   VITE_API_URL=http://130.49.148.127:8000/api
   ```

   (Замените `130.49.148.127` на IP или домен вашего сервера)

3. Перезапустите dev-сервер (`npm run dev`).

**Почему `127.0.0.1` не работает?** Адрес `127.0.0.1` на вашем ПК указывает на ваш собственный компьютер, а не на сервер. Бэкенд на сервере недоступен по этому адресу.
