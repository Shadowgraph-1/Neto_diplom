...
def register_user(request):
    ...
    logger.info(f"Попытка регистрации пользователя: {request.data.get('username', 'unknown')}")
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        if self.request.user.is_admin:
            logger.debug(f"Администратор {self.request.user.username} запрашивает список пользователей")
            return User.objects.all()
        logger.debug(f"Пользователь {self.request.user.username} запрашивает свою информацию")
        return User.objects.filter(id=self.request.user.id)
    
    def update(self, request, *args, **kwargs):
        """Обновление пользователя (изменение прав администратора)"""
        if not request.user.is_admin:
            logger.warning(f"Попытка обновления пользователя без прав администратора: {request.user.username}")
        ...
        if old_is_admin != user_to_update.is_admin:
            logger.info(f"Изменение прав администратора для {user_to_update.username}: {old_is_admin} -> {user_to_update.is_admin} администратором: {request.user.username}")
        
        return result
    
    def destroy(self, request, *args, **kwargs):
        if not request.user.is_admin:
            logger.warning(f"Попытка удаления пользователя без прав администратора: {request.user.username}")
            return Response({'error': 'Недостаточно прав'}, status=status.HTTP_403_FORBIDDEN)
        user_to_delete = self.get_object()
        logger.info(f"Удаление пользователя: {user_to_delete.username} (ID: {user_to_delete.id}) администратором: {request.user.username}")
...
class FileViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not uploaded_file:
            logger.warning(f"Попытка загрузки без файла пользователем: {request.user.username}")
            return Response({'error': 'Файл не предоставлен'}, status=status.HTTP_400_BAD_REQUEST)
        
        logger.info(f"Начало загрузки файла: {uploaded_file.name} (размер: {uploaded_file.size} байт) пользователем: {request.user.username}")
        ...
        try:
            ...
            logger.info(f"Файл успешно загружен: {uploaded_file.name} (ID: {file_obj.id}) пользователем: {request.user.username}")
    ...
    def download(self, request, pk=None):
        ...
        logger.info(f"Попытка скачивания файла: {file_obj.original_name} (ID: {file_obj.id}) пользователем: {request.user.username}")
        ...
        logger.info(f"Файл успешно скачан: {file_obj.original_name} пользователем: {request.user.username}")
    ...
    def view(self, request, pk=None):
        ...
        logger.info(f"Попытка просмотра файла: {file_obj.original_name} (ID: {file_obj.id}) пользователем: {request.user.username}")
        ...
        logger.info(f"Файл открыт для просмотра: {file_obj.original_name} пользователем: {request.user.username}")
        return response
    
    def update(self, request, *args, **kwargs):
        """Обновление файла (переименование, изменение комментария)"""
        file_obj = self.get_object()
        logger.info(f"Обновление файла: {file_obj.original_name} (ID: {file_obj.id}) пользователем: {request.user.username}")
        ...
        if 'comment' in request.data:
            ...
            logger.info(f"Изменение комментария к файлу {file_obj.original_name}: '{old_comment}' -> '{file_obj.comment}'")
    ...
    def destroy(self, request, *args, **kwargs):
        """Удаление файла"""
        file_obj = self.get_object()
        logger.info(f"Удаление файла: {file_obj.original_name} (ID: {file_obj.id}) пользователем: {request.user.username}")
...
def download_by_link(request, special_link):
    ...
    try:
        ...
        if not os.path.exists(file_full_path):
            logger.error(f"Файл не найден на диске по ссылке {special_link}: {file_full_path}")
            raise Http404("Файл не найден")
        
        file_obj.last_download_date = timezone.now()
        file_obj.save()
        
        logger.info(f"Файл успешно скачан по специальной ссылке: {file_obj.original_name}")
        response = FileResponse(open(file_full_path, 'rb'))
        response['Content-Disposition'] = f'attachment; filename="{file_obj.original_name}"'
        return response
    except File.DoesNotExist:
        logger.warning(f"Попытка скачивания несуществующего файла по ссылке: {special_link}")
        raise Http404("Файл не найден")
